...
const GeneratorForm: React.FC<GeneratorFormProps> = ({ onSubmit, isLoading, onSignIn }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!inputs.companyName || !inputs.location || !inputs.phone) {
    ...
    <div className="max-w-4xl mx-auto px-6 font-light" style={{ fontFamily: '"Avenir Light", Avenir, sans-serif' }}>
      ...
      <div className="flex justify-between items-center mb-6 md:mb-8">
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm font-bold tracking-[0.2em] text-white uppercase">
            PrimeHub <span className="text-green-500">AI</span>
          </div>
        </div>
        {onSignIn && (
          <button
      ...
      <div className="mb-6 md:mb-10 text-center space-y-3">
        <h1 className="text-2xl md:text-4xl font-bold tracking-tight text-white leading-tight">
          ...
          custom landscaping website <br className="hidden md:block" />
          <span className="text-green-500 italic">under a minute.</span>
        </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6 md:gap-y-8">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-transparent border-b border-white/10 px-0 py-2 focus:outline-none focus:border-green-500 transition-all text-white placeholder:text-gray-700 text-base md:text-lg"
              />
            </div>
