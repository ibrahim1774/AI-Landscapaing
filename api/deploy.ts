...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        // 1. Create a persistent directory for this build (so VA can access it)
        const savedSitesDir = path.join(process.cwd(), 'saved_sites');
        if (!fs.existsSync(savedSitesDir)) {
            fs.mkdirSync(savedSitesDir, { recursive: true });
        }

        const tempDir = path.join(savedSitesDir, projectName + '-' + Date.now());
